...
import {
  ...
  getNftSaleContract,
  getOBridgeBunniesContract,
  getOBridgeSquadContract,
...
import { WNATIVE } from '@obridge/sdk'
...
export const useOBridgeBunnies = () => {
  const { data: signer } = useSigner()
  return useMemo(() => getOBridgeBunniesContract(signer), [signer])
}
...
export const useOBridgeSquadContract = () => {
  const { data: signer } = useSigner()
  return useMemo(() => getOBridgeSquadContract(signer), [signer])
}
...
// @ts-ignore
export function useContract(address: string | undefined, ABI: any, withSignerIfPossible = true): T | null {
...
// @ts-ignore
export function usePairContract(pairAddress?: string, withSignerIfPossible?: boolean): IOBridgePair | null {
  return useContract(pairAddress, IOBridgePairABI, withSignerIfPossible)
}
...
export function useZapContract(withSignerIfPossible = true) {
  return useContract(getZapAddress(), zapAbi, withSignerIfPossible)
}
