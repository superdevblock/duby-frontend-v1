...
import { OBridgeDayDatasResponse } from '../types'
 ...
 */
const OBRIDGE_DAY_DATAS = gql`
  query overviewCharts($startTime: Int!, $skip: Int!) {
    obridgeDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      date
      dailyVolumeUSD
      totalLiquidityUSD
    }
...
const getOverviewChartData = async (
  ...
  try {
    const { obridgeDayDatas } = await getMultiChainQueryEndPointWithStableSwap(
      chainName,
    ).request<OBridgeDayDatasResponse>(OBRIDGE_DAY_DATAS, {
      startTime: multiChainStartTime[chainName],
      skip,
    })
    const data = obridgeDayDatas.map(mapDayData)
