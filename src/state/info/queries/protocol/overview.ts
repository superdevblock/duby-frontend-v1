...
interface OverviewResponse {
  obridgeFactories: OBridgeFactory[]
  factories?: OBridgeFactory[]
}
/**
...
const getOverviewData = async (
  ...
  const factoryString = checkIsStableSwap() ? `factories` : `obridgeFactories`
  try {
...
const formatOBridgeFactoryResponse = (rawOBridgeFactory?: OBridgeFactory) => {
  if (rawOBridgeFactory) {
    return {
      totalTransactions: parseFloat(rawOBridgeFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawOBridgeFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawOBridgeFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const overviewData = formatOBridgeFactoryResponse(data?.obridgeFactories?.[0])
      const overviewData24 = formatOBridgeFactoryResponse(data24?.obridgeFactories?.[0])
      const overviewData48 = formatOBridgeFactoryResponse(data48?.obridgeFactories?.[0])
...
export const fetchProtocolData = async (chainName: MultiChainName, block24: Block, block48: Block) => {
  ...
  if (data.factories && data.factories.length > 0) data.obridgeFactories = data.factories
  if (data24.factories && data24.factories.length > 0) data24.obridgeFactories = data24.factories
  if (data48.factories && data48.factories.length > 0) data48.obridgeFactories = data48.factories
