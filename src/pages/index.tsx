...
import { FACTORY_ADDRESS } from '@obridge/sdk'
...
export const getStaticProps: GetStaticProps = async () => {
  const totalTxQuery = gql`
    query TotalTransactions($id: ID!, $block: Block_height) {
      obridgeFactory(id: $id, block: $block) {
        totalTransactions
      }
    }
  `
  ...
  if (process.env.SF_HEADER) {
    try {
      ...
      if (
        totalTx?.obridgeFactory?.totalTransactions &&
        totalTx30DaysAgo?.obridgeFactory?.totalTransactions &&
        parseInt(totalTx.obridgeFactory.totalTransactions) > parseInt(totalTx30DaysAgo.obridgeFactory.totalTransactions)
      ) {
        results.totalTx30Days =
          parseInt(totalTx.obridgeFactory.totalTransactions) -
          parseInt(totalTx30DaysAgo.obridgeFactory.totalTransactions)
      }
    } catch (error) {
  ...
  const usersQuery = gql`
    query userCount($since: ISO8601DateTime, $till: ISO8601DateTime) {
      ethereum(network: bsc) {
        dexTrades(exchangeName: { in: ["OBridge", "OBridge v2"] }, date: { since: $since, till: $till }) {
          count(uniq: senders)
        }
      }
    }
  ...
  try {
    ...
    `)
    const { totalLiquidityUSD } = result.obridgeFactories[0]
